## Improvements and personalizations
⚡️ Add a Light/Dark mode functionality. The script is able to automatically set the theme according to the time (if not present a previous user choice in the local storage).\
⚡️ Some of the projects runs live into an iframe, so that is possible to immediately try them.\
⚡️ Resume shows off into a modal window.\
⚡️ Re-designed contact email section, showing email and button to copy it (in case mailto: fails to launch the mail client).\
